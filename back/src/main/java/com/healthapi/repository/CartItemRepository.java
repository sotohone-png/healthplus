...
import com.healthapi.domain.CartItem;
import com.healthapi.dto.CartItemListDTO;

public interface CartItemRepository extends JpaRepository<CartItem, Long>{

    @Query("select " + 
    " new com.healthapi.dto.CartItemListDTO(ci.cino,  ci.qty,  p.pno, p.pname, p.price , pi.fileName )  " +
    " from " +
    ...
    @Query("select new com.healthapi.dto.CartItemListDTO(ci.cino,  ci.qty,  p.pno, p.pname, p.price , pi.fileName )  " +
    " from " + 
    "   CartItem ci inner join Cart mc on ci.cart = mc " +
    "   left join Product p on ci.product = p " +
    "   left join p.imageList pi with pi.ord = 0" +
